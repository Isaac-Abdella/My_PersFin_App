...
import * as financialPlanner from "../utils/financialPlanner";
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const plan = financialPlanner.generateFinancialPlan(
      currentAge,
      retirementAge,
      currentIncome,
      currentSavings,
...
router.get("/", async (req, res, next) => {
  try {
    const userId = (req.user as any)._id;
    const plans = await FinancialPlan.find({ userId }).sort({ createdAt: -1 });
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const plan = await FinancialPlan.findOne({ _id: id, userId });
...
router.post("/retirement-analysis", async (req, res, next) => {
  try {
    ...
    // Analyze retirement readiness
    const analysis = financialPlanner.analyzeRetirementReadiness(
      projectedNetWorth,
      desiredAnnualIncome,
      cppMonthly,
      oasMonthly
...
router.post("/emergency-fund", async (req, res, next) => {
  try {
    ...
    const analysis = financialPlanner.analyzeEmergencyFund(
      monthlyExpenses,
      currentSavings,
      targetMonths
    );
...
router.post("/net-worth", async (req, res, next) => {
  try {
    ...
    const nw = financialPlanner.calculateNetWorth(
      assets,
      liabilities
    );
  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        totalLifetimeEarnings: Math.round(
          projections.reduce((sum, p) => sum + p.income, 0)
        ),
        averageAnnualIncome: Math.round(
