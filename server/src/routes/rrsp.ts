...
const ANNUAL_LIMIT_2024 = 31_560;
const ANNUAL_LIMIT_2025 = 32_490;

function currentAnnualLimit() {
  const yr = new Date().getFullYear();
  if (yr >= 2025) return ANNUAL_LIMIT_2025;
...
function computeSummary(accounts: any[]) {
  ...
  for (const acct of accounts) {
    ...
    if ((acct.deductionLimit ?? 0) > deductionLimit) deductionLimit = acct.deductionLimit;

    for (const c of acct.contributions) {
      totalContributed += c.amount;
      if (c.year === currentYear) thisYearContrib += c.amount;
    }
  ...
  if (deductionLimit === 0) deductionLimit = currentAnnualLimit();
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const accounts = await RRSPAccount.find({ userId }).sort({ createdAt: 1 });
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const account = await RRSPAccount.create({
      userId,
      accountName,
...
router.put("/:id", async (req: Request, res: Response) => {
  try {
    const userId = (req.user as any).id;
    const { accountName, institution, balance, deductionLimit, isAccountOwner, notes } = req.body;
    const account = await RRSPAccount.findOneAndUpdate(
      { _id: req.params.id, userId },
      { accountName, institution, balance: Number(balance), deductionLimit: Number(deductionLimit), isAccountOwner: isAccountOwner !== false, notes },
      { new: true }
    );
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    const userId  = (req.user as any).id;
    const account = await RRSPAccount.findOneAndDelete({ _id: req.params.id, userId });
    if (!account) return res.status(404).json({ message: "Not found" });
