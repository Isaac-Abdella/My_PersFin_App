...
const CANADIAN_TEMPLATE_CATEGORIES: Array<{ category: string; amount: number }> = [
...
const CATEGORY_META_BY_NAME = new Map<string, MajorMeta>();
const CATEGORY_NAME_BY_KEY = new Map<string, string>();
for (const major of CATEGORY_CATALOG) {
...
function getPeriodRange(cycle: BudgetCycle, referenceDate: Date, paydayAnchor?: Date) {
...
function getPreviousPeriodRange(cycle: BudgetCycle, currentStart: Date) {
...
function getRolloverAmount(mode: RolloverMode, budgetAmount: number, previousSpent: number): number {
  const delta = budgetAmount - previousSpent;
  if (mode === "none") return 0;
  if (mode === "carry-unused") return Math.max(0, delta);
  return delta;
}
...
function getMajorMeta(category: string, categoryKey?: string): MajorMeta {
...
router.post("/templates/canada/apply", async (req: Request, res: Response) => {
  try {
    ...
    const existing = await Budget.find({ userId, period, isActive: true }, { category: 1 }).lean();
...
router.post("/major-plan", async (req: Request, res: Response) => {
  try {
    ...
    const major = CATEGORY_CATALOG.find((m) => m.key === majorCategoryKey);
    if (!major) {
    ...
    const normalizedAllocations = allocations
      .map((a: any) => {
        ...
        if (!subName || !isFinite(amount) || amount < 0) return null;
      ...
      .filter(Boolean) as Array<{ subcategoryName: string; subcategoryKey: string; amount: number }>;
    ...
    const upserted = [];
    const includedSubcategoryNames = new Set<string>();
...
router.delete("/major-plan/:majorCategoryKey", async (req: Request, res: Response) => {
  try {
    ...
    const period = ((req.query.period as string) || "monthly") as BudgetCycle;
...
router.get("/summary", async (req: Request, res: Response) => {
  try {
    const userId = (req.user as any).id;
    const cycle = ((req.query.cycle as string) || "monthly") as BudgetCycle;
    ...
    const paydayAnchor = req.query.paydayAnchor ? new Date(req.query.paydayAnchor as string) : undefined;

    const { start, end } = getPeriodRange(cycle, referenceDate, paydayAnchor);
    const previous = getPreviousPeriodRange(cycle, start);

    const budgets = await Budget.find({ userId, period: cycle, isActive: true }).sort({ category: 1 });
    ...
    for (const budget of budgets) {
      const majorMeta = getMajorMeta(budget.category, (budget as any).categoryKey);
      const majorKey = (budget as any).majorCategoryKey || majorMeta.majorCategoryKey;
      ...
      const currentSpent = currentTxns.reduce((sum: number, t: any) => sum + t.amount, 0);
      const previousSpent = previousTxns.reduce((sum: number, t: any) => sum + t.amount, 0);

      // Prevent first-cycle double counting: only roll over if budget existed before current period.
      const budgetStartDate = new Date((budget as any).startDate);
      const hasPriorCycle = budgetStartDate < start;
      const rolloverAmount = hasPriorCycle
        ? getRolloverAmount(
            (budget.rolloverMode || "carry-unused") as RolloverMode,
            budget.amount,
            previousSpent
          )
        : 0;
      ...
      items.push({
        ...
        majorCategoryKey: (budget as any).majorCategoryKey || majorMeta.majorCategoryKey,
        majorCategoryName: (budget as any).majorCategoryName || majorMeta.majorCategoryName
      });
    }

    const majorSummaryMap: Record<string, any> = {};
    ...
    const majorSummaries = Object.values(majorSummaryMap).map((m: any) => ({
      ...m,
...
router.get("/payday-plan", async (req: Request, res: Response) => {
  try {
    const userId = (req.user as any).id;
    const cycle = ((req.query.cycle as string) || "biweekly") as BudgetCycle;
    ...
    const budgets = await Budget.find({ userId, period: cycle, isActive: true }).lean();

    const allocations = [] as Array<{ category: string; suggested: number; kind: "needs" | "wants" }>;
    ...
    const remainingByBudget = await Promise.all(
      budgets.map(async (budget: any) => {
        ...
        const spent = txns.reduce((sum: number, t: any) => sum + t.amount, 0);
