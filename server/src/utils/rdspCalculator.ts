 ...
 */
export function calculateCCESGAvailable(
  annualContribution: number,
  lifetimeGrantsReceived: number
): {
  maxMatch: number;
  grantAmount: number;
  matchRatio: number;
} {
  const ccsgLimit = RDSP_LIMITS_2024.ccesgLifetimeLimit;
  ...
  const baseMatch = Math.min(
    annualContribution * 3.5,
    RDSP_LIMITS_2024.ccesgAnnualLimit
  );
 ...
 */
export function calculateCDBSAvailable(
  familyNetIncome: number,
  lifetimeBondsReceived: number,
  yearsSinceLastBond: number = 0
): {
  bondsPayable: number;
  carryForwardBonds: number;
  reason: string;
} {
  const cdbsLimit = RDSP_LIMITS_2024.cdbsLifetimeLimit;
  ...
  if (familyNetIncome <= thresholdLow) {
    ...
    const reduction =
      ((familyNetIncome - thresholdLow) / (thresholdHigh - thresholdLow)) * 1500;
 ...
 */
export function projectRDSPGrowth(
  ...
  for (let year = 1; year <= years; year++) {
    ...
    let yearGrants = 0;
    if (cumulativeGrants < RDSP_LIMITS_2024.ccesgLifetimeLimit) {
      yearGrants = Math.min(
        annualGrants,
        RDSP_LIMITS_2024.ccesgLifetimeLimit - cumulativeGrants
      );
    ...
    let yearBonds = 0;
    if (cumulativeBonds < RDSP_LIMITS_2024.cdbsLifetimeLimit) {
      yearBonds = Math.min(
        annualBonds,
        RDSP_LIMITS_2024.cdbsLifetimeLimit - cumulativeBonds
      );
 ...
 */
export function calculateLifetimeAccumulation(
  currentAge: number,
  targetWithdrawalAge: number,
  currentBalance: number,
  annualContribution: number,
  maxAnnualGrants: number,
  maxAnnualBonds: number,
 ...
 */
export function calculateMaxLDAP(
  accountBalance: number,
  age: number,
 ...
 */
export function analyzeContributionStrategy(
  targetBalance: number,
  yearsToTarget: number,
  currentBalance: number,
  expectedAnnualGrants: number,
  expectedAnnualBonds: number,
  averageReturnRate: number
): {
  requiredAnnualContribution: number;
  withGovernmentSupport: number;
  projectedBalance: number;
  ...
  const futureValueOfCurrent =
    currentBalance * Math.pow(1 + ratePerYear, yearsToTarget);
  const futureValueOfGovSupport =
    governmentSupport *
    (Math.pow(1 + ratePerYear, yearsToTarget) - 1) / ratePerYear;

  const requiredFromContributions =
    targetBalance - futureValueOfCurrent - futureValueOfGovSupport;
  const requiredAnnualContribution = Math.max(
    0,
  ...
  if (requiredAnnualContribution <= 0) {
    ...
    recommendations.push(
      `💡 Contribute $${Math.round(requiredAnnualContribution)}/year to reach target`
    );
  } else {
    recommendations.push(
      `⚠️ Would need $${Math.round(requiredAnnualContribution)}/year (max allowed: $2,500)`
    );
