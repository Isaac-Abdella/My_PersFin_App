...
exports.getETFRecommendations = getETFRecommendations;
exports.calculateMonthlyInvestmentNeeded = calculateMonthlyInvestmentNeeded;
...
function generateInvestmentRecommendation(currentNetWorth, goalAmount, goalYear, currentAge, retirementAge) {
    ...
    const investmentCalc = calculateMonthlyInvestmentNeeded(currentNetWorth, goalAmount, yearsToGoal, avgReturn);
    // Project growth
    const projections = projectPortfolioGrowth(currentNetWorth, investmentCalc.monthlyInvestment, avgReturn, yearsToGoal, currentAge);
    // Calculate success probability
    const successProbability = calculateSuccessProbability(currentNetWorth, investmentCalc.monthlyInvestment, goalAmount, yearsToGoal, riskProfile);
    // Generate recommendations
    const recommendations = [];
    if (successProbability < 50) {
        recommendations.push(`⚠️ Current plan has only ${successProbability}% success probability. Consider: (1) increasing monthly contributions, (2) extending timeline, or (3) reassessing goal amount.`);
    }
    ...
    if (riskProfile === "aggressive" && yearsToGoal < 5) {
        recommendations.push(`📊 Consider gradually shifting to a more conservative allocation as you approach your goal (${goalYear}).`);
    }
    recommendations.push(`💡 Average Canadian ETF MER is 0.20%. These recommendations use 0.08-0.16%.`);
    recommendations.push(`📈 Rebalance portfolio annually to maintain target allocation and stay discipline during market volatility.`);
    recommendations.push(`🏦 Consider maxing out TFSA ($7,000/year) before RRSP for tax-free growth on investments.`);
    return {
        riskProfile,
        allocation,
        etfs,
