...
async function getLatestImportedStatementBalance(userId, accountId) {
    const latestStatementTxn = await Transaction_1.Transaction.findOne({
        userId,
        accountId,
        statementBalance: { $ne: null }
    }).sort({ date: -1, createdAt: -1, _id: -1 });
    if (!latestStatementTxn)
        return null;
    const value = latestStatementTxn.statementBalance;
...
router.post("/upload", upload.single("file"), async (req, res) => {
    try {
        ...
        let normalizedSkipDuplicateIds = [];
        ...
        const existingTransactions = await Transaction_1.Transaction.find({ userId, accountId }, { date: 1, amount: 1, description: 1 }).lean();
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            try {
                ...
                if (!transaction.category) {
                    transaction.category = (0, categorization_1.categorizeTransaction)(transaction.description || "");
                }
        ...
        for (const record of parsedRecords) {
            ...
            try {
                const saved = await Transaction_1.Transaction.create(record.transaction);
...
router.get("/duplicates/:accountId", async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const transactions = await Transaction_1.Transaction.find({ userId, accountId }).sort({ date: 1 });
        ...
        const serializedGroups = duplicateGroups.map((group) => group.map((tx) => ({
...
// Detect CSV format and map columns
function detectCSVFormat(records) {
    const firstRow = records[0];
    const headers = Object.keys(firstRow).map(h => h.toLowerCase());
    // Common date column names
    const dateCol = headers.find(h => h.includes("date") || h === "posted" || h === "transaction date" || h === "trans. date");
    // Common description column names - including the TD Bank typo "desciption"
    const descCol = headers.find(h => h.includes("description") || h.includes("desciption") || h.includes("memo") ||
        h.includes("merchant") || h === "name" || h.includes("payee"));
    // Amount columns - could be single amount or separate debit/credit
    const amountCandidates = headers.filter(h => h === "amount" ||
        h.includes("amount") ||
        h.includes("amt") ||
        h.includes("charge") ||
    ...
    const typeCol = headers.find(h => h === "type" ||
    ...
    const getOriginalName = (lowerName) => {
        return Object.keys(firstRow).find(k => k.toLowerCase() === lowerName) || lowerName;
    };
    const scoreAmountHeader = (lowerHeader) => {
        let score = 0;
        if (lowerHeader === "amount")
            score += 40;
        if (lowerHeader.includes("cad") || lowerHeader.includes("local"))
            score += 35;
        if (lowerHeader.includes("account"))
            score += 15;
        if (lowerHeader.includes("usd") || lowerHeader.includes("foreign") || lowerHeader.includes("original"))
            score -= 25;
...
function parseCSVRow(row, mapping, userId, accountId) {
    ...
    if (mapping.amountCol) {
        ...
        const candidateAmountCols = Array.isArray(mapping.amountCols) && mapping.amountCols.length > 0
            ? mapping.amountCols
            : [mapping.amountCol];
    ...
    return {
        ...
        description,
        category,
        statementBalance: isFinite(statementBalance) ? statementBalance : null,
...
function parseAmountValue(rawValue) {
    ...
    if (!isFinite(parsed))
        return NaN;
    let sign = 1;
    if (hasParensNegative || hasTrailingNegative || hasLeadingNegative || hasDr)
        sign = -1;
    if (hasCr)
        sign = 1;
    return parsed * sign;
}
function parseDirectionType(rawDirection) {
...
router.post("/statement", upload.single("statement"), async (req, res) => {
    try {
        ...
        const isFilePdf = req.file.mimetype === "application/pdf" ||
            req.file.originalname.toLowerCase().endsWith(".pdf");
        if (!isFilePdf)
            return res.status(400).json({ message: "Only PDF files are supported for statement import" });
        // Enforce a 20 MB cap
        ...
        // Parse statement
        const result = (0, pdfStatementParser_1.parseStatement)(pdfText);
        ...
        const existingTxns = await Transaction_1.Transaction.find({ userId, accountId }, { date: 1, amount: 1, description: 1 }).lean();
        const existingKeys = new Set(existingTxns.map(tx => `${new Date(tx.date).toDateString()}-${tx.amount}-${tx.description}`));
        let imported = 0;
        let skipped = 0;
        const errors = [];
