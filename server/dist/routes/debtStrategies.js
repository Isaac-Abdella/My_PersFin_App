...
const DebtStrategy_1 = require("../models/DebtStrategy");
const requireLogin_1 = require("../middleware/requireLogin");
const debtOptimizer = __importStar(require("../utils/debtOptimizer"));
...
router.post("/analyze", async (req, res, next) => {
    try {
        ...
        // Fetch all debts
        const debts = await Debt_1.Debt.find({ _id: { $in: debtIds }, userId });
        if (debts.length === 0) {
        ...
        const priorityOrder = plan.priorityOrder.map((p) => ({
            debtId: new mongoose_1.default.Types.ObjectId(p.debtId),
            debtName: p.debtName,
        ...
        const strategy = await DebtStrategy_1.DebtStrategy.create({
            userId,
...
router.get("/", async (req, res, next) => {
    try {
        const userId = req.user._id;
        const strategies = await DebtStrategy_1.DebtStrategy.find({ userId })
            .populate("debtIds")
            .sort({ createdAt: -1 });
        res.json({ strategies });
    }
...
router.get("/:id", async (req, res, next) => {
    try {
        const userId = req.user._id;
        const { id } = req.params;
        const strategy = await DebtStrategy_1.DebtStrategy.findOne({ _id: id, userId }).populate("debtIds");
        if (!strategy) {
...
router.post("/consolidation-analysis", async (req, res, next) => {
    try {
        ...
        const totalMinimumPayment = debts.reduce((sum, d) => sum + d.minimumPayment, 0);
        const extraPayment = Math.max(0, monthlyBudget - totalMinimumPayment);
        const analysis = debtOptimizer.analyzeConsolidation(debts, consolidationRate, consolidationFee, extraPayment);
        res.json({
            currentPlan: {
...
router.post("/mortgage-acceleration", async (req, res, next) => {
    try {
        ...
        const debt = await Debt_1.Debt.findOne({ _id: debtId, userId });
        if (!debt) {
            return res.status(404).json({ message: "Mortgage not found" });
        }
        const result = debtOptimizer.calculateMortgageAcceleration(debt, accelerationMethod, accelerationAmount);
        res.json({
            ...
            recommendation: result.yearsSaved > 2
                ? `🎉 This strategy saves $${result.interestSavings.toFixed(2)} and ${result.yearsSaved.toFixed(1)} years! Highly recommended.`
                : `⚠️ Savings of $${result.interestSavings.toFixed(2)} over ${result.yearsSaved.toFixed(1)} years. May not be worth the extra payments.`,
        });
    }
    catch (err) {
        next(err);
    ...
    try {
        ...
        res.json({
            ...
            savings: {
                ...
                recommendation: `If you pay an extra $${extraMonthlyPayment}/month, you'll be debt-free ${result.monthsSaved} months earlier and save $${result.interestSaved.toFixed(2)} in interest!`,
            },
        });
    }
    catch (err) {
...
router.get("/recommendations/analysis", async (req, res, next) => {
    try {
        ...
        const debtIdArray = debtIds.split(",");
        const debts = await Debt_1.Debt.find({ _id: { $in: debtIdArray }, userId });
        if (debts.length === 0) {
            return res.status(404).json({ message: "No debts found" });
        }
        const totalMinimumPayment = debts.reduce((sum, d) => sum + d.minimumPayment, 0);
        ...
        const recommendations = debtOptimizer.getDebtRecommendations(debts, avalanche, snowball, Number(income) || 0);
        res.json({
            debts: debts.map((d) => ({
                name: d.name,
                type: d.type,
            ...
            summary: {
                ...
                totalMinimumPayment,
                averageRate: (debts.reduce((sum, d) => sum + d.interestRate, 0) / debts.length).toFixed(1),
            },
            strategies: {
                avalanche: {
                    payoffMonths: avalanche.payoffMonths,
